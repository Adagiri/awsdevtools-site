...
export function ToolDashboardPage({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    const loadToolData = async () => {
      ...
      setToolData(TOOL_DATA[slug] || null);
      setIsLoading(false);
    };
  ...
  const handleRunTool = async () => {
    ...
    try {
      setIsRunning(true);
      await runTool(toolData.name);
      // Refresh data after run
      await new Promise(resolve => setTimeout(resolve, 2000));
      setToolData(prev => prev ? { ...prev, lastRun: new Date().toISOString() } : null);
    } catch (error) {
        ...
        <div className="flex">
          ...
          <main className="flex-1 p-4 md:p-8">
            <div className="text-center py-12">
              ...
              <p className="text-slate-400 mb-6">The tool dashboard you're looking for doesn't exist.</p>
              <Link href="/tools">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 md:mb-8 space-y-4 md:space-y-0">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className="text-2xl md:text-3xl font-bold text-white">{toolData.displayName}</h1>
                  <p className="text-slate-400">{toolData.description}</p>
                </div>
              </div>
            </div>
            <div className="flex items-center space-x-3">
              <Link href={`/tools/${slug}/settings`}>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  ...
                  <div className="text-slate-400">
                    Last run: {toolData.lastRun ? formatTimeAgo(toolData.lastRun) : 'Never'}
                  </div>
                </div>
                <div className="flex items-center space-x-4">
          ...
          <Tabs defaultValue="overview" className="space-y-6">
            ...
            <TabsContent value="overview" className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {toolData.metrics.map((metric, index) => (
                  ...
                  <CardContent>
                    <div className="h-64 flex items-center justify-center bg-slate-900/30 rounded-lg">
                      <div className="text-center">
                        ...
                        <p className="text-slate-400">Pie chart visualization would go here</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                    ...
                    {[
                      ...
                      <div key={index} className="flex items-center justify-between p-3 bg-slate-900/30 rounded-lg">
                        <div className="flex items-center space-x-3">
                          <CheckCircle className="w-5 h-5 text-green-400" />
                          <div>
                            <div className="text-white font-medium">Analysis completed</div>
